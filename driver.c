...
void statement(lexeme *list)
{
    ...
    if(list[tok_index].type==beginsym)
    {
        do
        {
            tok_index++;
        ...
        if(list[tok_index].type!=endsym)
        {
            if((list[tok_index].type==identsym)||(list[tok_index].type==beginsym)||(list[tok_index].type==ifsym)||(list[tok_index].type==whilesym)||(list[tok_index].type==readsym)||(list[tok_index].type==writesym)||(list[tok_index].type==callsym))
                printparseerror(16);
            else
                printparseerror(15);
        }
    ...
    if(list[tok_index].type==ifsym)
    {
        ...
        if(list[tok_index].type!=thensym)
            printparseerror(8);
        
        tok_index++;
        statement(list);
        if(list[tok_index].type==elsesym)
        {
            int jmpIdx = cIndex;
